...
def crop_with_overlap(image, save_path, tile_size=(512, 512), stride=(256, 256)):
    tiles = []
    save_path = str(save_path)  # Ensure save_path is a string path

    # Make sure the directory exists if saving tiles
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    ...
    for i in range(0, image.width - tile_size[0] + 1, stride[0]):
        for j in range(0, image.height - tile_size[1] + 1, stride[1]):
            ...
            # Create a file path to save the cropped tiles (if needed)
            tile_path = os.path.join(save_path, f"tile_{i}_{j}.jpg")
            tile.save(tile_path)
            tiles.append(tile_path)
...
def classify_and_aggregate(learn_inf, image, save_path, tile_size=(512, 512), stride=(256, 256)):
    # Crop the image into tiles
    tiles = crop_with_overlap(image, save_path, tile_size, stride)

    # Predict on each tile
    predictions = [(os.path.basename(tile), learn_inf.predict(PILImage.create(tile))) for tile in tiles]
